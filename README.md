...
1. installer_PDE-ARF_ver1.1.0.exe をダブルクリックして起動します。
2. 警告画面が出ることがあるので「はい」を押して許可します。
3. 【インストール先の指定】特に何もなければそのまま「次へ」を押します。
4. 【追加タスクの選択】デスクトップ上にショートカットキーを作るか確認されるので設定します（作成を推奨）。
5. 【インストール準備完了】「インストール」を押します。
6. インストールが完了したら、会計要望書生成を起動する場合は「会計要望書生成 を実行する」にチェックを入れます。
7. 「完了」を押します。
8. インストール完了！
